...
function db(system,cfgDir) {
	...
	try {
		var data = fs.readFileSync(path.join(cfgDir, 'db'));
	...
	system.on('db_save', function () {
		...
		if (Date.now() - lastsave > saveInterval) {
			...
			fs.copy(path.join(cfgDir, 'db'), path.join(cfgDir, 'db.bak'), function (err) {
				if (err) {
					debug('db_save', 'Error making backup of config: ' + err);
				}

				fs.writeFile(path.join(cfgDir, 'db.tmp'), JSON.stringify(self.db), function (err) {
					if (err) {
					...
					fs.rename(path.join(cfgDir, 'db.tmp'), path.join(cfgDir, 'db'), function(err) {
